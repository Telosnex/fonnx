...
void main() {
  ...
  test('matches the empty-string embedding golden', () async {
    await expectEmbeddingMatchesGolden(
      vec,
      'test/data/goldens/potion32m_empty.json',
    );
  });
  ...
  test('matches a non-empty embedding golden', () async {
    await expectEmbeddingMatchesGolden(
      vec,
      'test/data/goldens/potion32m_rain_in_spain.json',
    );
  });
  ...
  test('Divided long text into chunks', () async {
    ...
    expect(allEmbeddings[0].text, hasLength(802));
    expect(allEmbeddings[1].text, hasLength(761));
    expect(allEmbeddings[12].text, hasLength(288));
  });
  ...
  test('Performance test', () async {
    ...
    for (var i = 0; i < count; i++) {
      final future = minishLab.getEmbeddingAsVector(tokens[i % tokens.length]);
      futures.add(future);
    }
  ...
  test('Similarity', () async {
    final result1 = await minishLab.getEmbeddingAsVector(tokenize('Bonjour'));
    final result2 = await minishLab.getEmbeddingAsVector(tokenize('Ni hao'));
    final result = result1.cosineSimilarity(result2);
    expect(result, closeTo(0.002, 0.001));
  });
  ...
  test('Similarity: weather', () async {
    ...
    expect(sRandomToAnswer, closeTo(0.047, 0.001));
    expect(sSFToAnswer, closeTo(0.057, 0.001));
    expect(sWFInBuffaloToAnswer, closeTo(0.376, 0.001));
    expect(sWFToAnswer, closeTo(0.533, 0.001));
    expect(sSpainWFToAnswer, closeTo(0.621, 0.001));
    expect(sWFInSpainToAnswer, closeTo(0.634, 0.001));
  });
  ...
  test('Similarity: password', () async {
    ...
    expect(vQuery.cosineSimilarity(vAnswer), closeTo(0.296, 0.001));
    final vRandom = await vec('Rain in Spain falls mainly on the plain');
    expect(vQuery.cosineSimilarity(vRandom), closeTo(0.053, 0.001));
  });
  ...
  test('Similarity: London', () async {
    ...
    expect(vQuery.cosineSimilarity(vAnswer), closeTo(0.063, 0.001));
  });
}
