...
void main() {
  ...
  final tokenizer = MinishLab.potion8mTokenizer;
  ...
  test('Performance test', () async {
    final List<List<int>> tokens = MinishLab.potion8mTokenizer
        .tokenize(data)
        .map((e) => e.tokens)
        .toList();
    const count = 100;
    ...
    for (var i = 0; i < count; i++) {
      final thisTokens = tokens[i % tokens.length];
      final future = minishLab.getEmbeddingAsVector(thisTokens);
      futures.add(future);
    }
  ...
  test('Similarity', () async {
    ...
    expect(result, closeTo(0.115, 0.001));
  });
  ...
  test('Similarity: weather', () async {
    ...
    expect(sRandomToAnswer, closeTo(0.059, 0.001));
    expect(sSFToAnswer, closeTo(0.337, 0.001));
    expect(sWFInBuffaloToAnswer, closeTo(0.560, 0.001));
    expect(sWFToAnswer, closeTo(0.684, 0.001));
    expect(sSpainWFToAnswer, closeTo(0.742, 0.001));
  ...
  test('Similarity: password', () async {
    ...
    expect(vQuery.cosineSimilarity(vAnswer), closeTo(0.171, 0.001));
    final vRandom = await vec('Rain in Spain falls mainly on the plain');
    expect(vQuery.cosineSimilarity(vRandom), closeTo(0.072, 0.001));
  });
  ...
  test('Similarity: London', () async {
    ...
    expect(vQuery.cosineSimilarity(vAnswer), closeTo(0.181, 0.001));
  });
}
